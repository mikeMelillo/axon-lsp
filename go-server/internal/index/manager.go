...
func fromTrio(parsed map[string]trio.ParsedFunction) (map[string]Symbol, []DocumentSymbol) {
	...
	for _, symbol := range parsed {
		...
		selectionRange := Range{Start: Position{Line: symbol.StartLine, Character: symbol.StartChar}, End: Position{Line: symbol.EndLine, Character: symbol.EndChar}}
		rangeValue := Range{Start: selectionRange.Start, End: Position{Line: symbol.ScopeEndLine, Character: symbol.ScopeEndChar}}
		docSymbols = append(docSymbols, DocumentSymbol{Name: symbol.Name, Detail: symbol.ArgsStr, Kind: symbolKindFunction, Range: rangeValue, SelectionRange: selectionRange})
	}
...
func fromXeto(parsed map[string]xeto.ParsedFunction) (map[string]Symbol, []DocumentSymbol) {
	...
	for _, symbol := range parsed {
		result[symbol.Name] = Symbol{
			...
			Location: &Location{URI: symbol.URI, Range: Range{
				Start: Position{Line: symbol.StartLine, Character: symbol.StartChar},
				End:   Position{Line: symbol.StartLine, Character: symbol.StartChar + len(symbol.Name)},
			}},
			Origin:      OriginLocal,
			SourceKind:  "workspace",
			SourceModel: "specs",
		...
		rangeValue := Range{Start: Position{Line: symbol.StartLine, Character: symbol.StartChar}, End: Position{Line: symbol.EndLine, Character: symbol.EndChar}}
		selectionRange := Range{Start: rangeValue.Start, End: Position{Line: symbol.StartLine, Character: symbol.StartChar + len(symbol.Name)}}
		docSymbols = append(docSymbols, DocumentSymbol{Name: symbol.Name, Detail: detailFor(result[symbol.Name]), Kind: symbolKindFunction, Range: rangeValue, SelectionRange: selectionRange})
	}
	sortDocumentSymbols(docSymbols)
	return result, docSymbols
}
