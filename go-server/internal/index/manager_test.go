...
`)
	...
	for _, symbol := range symbols {
		if symbol.Range.End.Line <= symbol.SelectionRange.End.Line {
			t.Fatalf("expected document symbol %s to span its Trio record: %#v", symbol.Name, symbol)
		}
	}
}
...
func TestSpecsModeIndexesXetoCallableFunctions(t *testing.T) {
  ...
  addExample: Func { a: Number, b: Number, returns: Number
	...
	if symbols[0].Range.End.Line <= symbols[0].SelectionRange.End.Line {
		t.Fatalf("expected Xeto document symbol to span the full Func block: %#v", symbols[0])
	}
	loc := mgr.GetDefinition("addExample")
	if loc == nil || loc.Range.End.Line != loc.Range.Start.Line {
		t.Fatalf("expected Xeto definition to select only the function name: %#v", loc)
	}
}
